...
public class ThresherContainer extends ATTContainerMenu
{
    ...
    public static ThresherContainer makeServer(MenuType<?> type, int id, Inventory playerInventory, MultiblockMenuContext<ThresherLogic.State> ctx)
    {
        final ThresherLogic.State state = ctx.mbContext().getState();
        return new ThresherContainer(
            multiblockCtx(type, id, ctx),
            playerInventory,
            state.getInventory(),
            state.getEnergy(),
            GetterAndSetter.getterOnly(() -> state.getProcessQueue().stream()
                .filter(p -> p instanceof ATTMultiblockProcess.ProcessWithItemStackProvider<ThresherRecipe>)
                .map(p -> ProcessSlot.fromCtx((ATTMultiblockProcess.ProcessWithItemStackProvider<ThresherRecipe>) p, ctx.mbContext().getLevel().getRawLevel()))
                .toList()
            )
        );
    }

    public static ThresherContainer makeClient(MenuType<?> type, int id, Inventory playerInventory)
    {
        return new ThresherContainer(
            clientCtx(type, id),
            playerInventory,
            new ItemStackHandler(ThresherLogic.NUM_SLOTS),
            new MutableEnergyStorage(ThresherLogic.ENERGY_CAPACITY),
            GetterAndSetter.standalone(List.of())
        );
    }

    public ThresherContainer(MenuContext ctx, Inventory playerInventory, IItemHandler inv, IMutableEnergyStorage energy, GetterAndSetter<List<ProcessSlot>> processes)
    {
        super(ctx);
        this.energy = energy;
        ...
        for (int i = 0; i < 6; i++)
        {
            this.addSlot(new SlotItemHandler(inv, i, 62 + 18 * (i % 3), 16 + 18 * (i / 3))
            {
                ...
                {
                    return ThresherRecipe.findRecipe(playerInventory.player.level(), stack) != null;
                }
            });
        }
        for (int i = 0; i < 6; i++)
        ...
        addGenericData(new ATTGenericContainerData<>(ATTGenericDataSerializers.THRESHER_PROCESS_SLOTS, processes));
    }

    public record ProcessSlot(int processStep)
    {
        public static ProcessSlot fromCtx(ATTMultiblockProcess.ProcessWithItemStackProvider<ThresherRecipe> process, Level level)
        {
