...
public class ConfigBuilder
{
    ...
    {
        return (Supplier) factory.create(begin(path).defineListAllowEmpty(path, new ArrayList<>(value), ArrayList::new, o -> true));
    }

    private ModConfigSpec.Builder begin(String path)
    {
        builder.translation("advancedtfctech.config." + translationKeyPrefix + "." + path);
...
//    record CommonValue<T>(ModConfigSpec.ConfigValue<T> value) implements Supplier<T>
//    {
//        @Override
//        public T get()
//        {
//            return ATTConfig.COMMON.spec().isLoaded() ? value.get() : value.getDefault();
//        }
//    }
    ...
    record ClientValue<T>(ModConfigSpec.ConfigValue<T> value) implements Supplier<T>
    {
        ...
        public T get()
        {
            return ATTConfig.CLIENT.spec().isLoaded() ? value.get() : value.getDefault();
        }
    }
    ...
    record ServerValue<T>(ModConfigSpec.ConfigValue<T> value) implements Supplier<T>
    {
        ...
        public T get()
        {
            return ATTConfig.SERVER.spec().isLoaded() ? value.get() : value.getDefault();
        }
    }
}
